<?php 
	...
	AUTHOR: Ethan Gruber
	MODIFIED: March 2026
	DESCRIPTION: Receive an accession or lot number in a parameter and delete from Numishare:
	eXist database and Solr index. It can work on multiple Numishare collections
	...
	************************/
	...
	if (file_exists($eXist_config_path)) {
	    ...
	    if (strlen($collection) > 0){
	        //delete accnum
	        if (strlen($accnum) > 0 && strlen($collection) > 0){
	            $basename = $accnum . '.xml';
	            ...
	            echo "No accession or lot number specified.\n";
	        }
	    } else {
	        echo "No collection parameter specified.\n";
	    }
	} else {
	    "Error: Unable to read eXist-db configuration.\n";
	}
